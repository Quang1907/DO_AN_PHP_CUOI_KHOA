...
<div class="sm:container my-5">
    ...
    <form class="space-y-6 w-1/2 m-auto" method="post" action="{{ route('admin/product/update', $product['id']) }}" enctype="multipart/form-data">
        @csrf
        <div class="text-3xl font-semibold text-center">Chỉnh sửa sản phẩm</div>
        <div class="relative z-0 mb-6 w-full group">
            <label class="block text-sm font-medium text-gray-900 dark:text-gray-300" for="multiple_files">Tên sản phẩm</label>
            <input type="text" name="product_name" value="{{ $product['product_name'] }}" id="product_name" class="block py-2 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="">
            {{ error("product_name") }}
        </div>
        <div class="relative z-0 mb-6 w-full group">
            <label class="block text-sm font-medium text-gray-900 dark:text-gray-300" for="multiple_files">Giá</label>
            <input type="number" name="price" id="price" value="{{ $product['price'] }}" class="block py-2 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="">
            {{ error("price") }}
        </div>
        <div class="overflow-hidden relative w-ful mt-4 mb-4">
            ...
            <img class="w-20 h-auto mt-2" src="{{ _WEB_ROOT .'images/products/'. $product['image'] }}" size="10px" alt="image description" id="showimage">
        ...
        <div class="overflow-hidden relative w-full mt-4 mb-4">
            ...
            <div id="showimages" class="flex flex-row flex-wrap gap-2 justify-center mt-2">
                <?php
                $listImage = explode(",", $product['images']);
                foreach ($listImage as $key => $img) {
                    echo '<img class="w-20 h-auto mt-2" src="' . _WEB_ROOT . "images/products/" . trim($img) . '" size="10px" alt="image description" id="showimage">';
                }
                ?>
            </div>
        ...
        <div class=" relative z-0 mb-6 w-full group">
            ...
            <div class="mb-4 w-full bg-gray-50 rounded-lg border border-gray-200 dark:bg-gray-700 dark:border-gray-600">
                <div class="py-2 px-4 bg-white rounded-b-lg dark:bg-gray-800">
                    ...
                    <textarea id="editor" name="descript" rows="8" class="block px-0 w-full text-sm text-gray-800 bg-white border-0 focus:ring-0" placeholder="Mô tả sản phầm">{{ $product["descript"] }}</textarea>
                    {{ error("descript") }}
                </div>
            </div>
        </div>
